...
In this document, we will walk through how to perform statistical downscaling, such as the kind performed by the Statistical Downscaling Model (SDSM) tool, in the R language. Statistical downscaling is a method of relating large scale variables, such as those from General Circulation Model (GCM) output, to locally observed variables. Statistical downscaling provides a method for performing local impact studies, such as assessing the risk of flooding in a particular city under different climate change scenario's. At a high level, what's being done is building a statistical model of the relationship between large scale variables (for example, temperature in a 50 x 50 km gridbox) and local variables using re-analysis data. Then, we use this statistical relationship to predict how the local variable responds to various climate scenario's.  

We will walk through the Blogsville example used in the SDSM manual, and show how you can perform these calculations in the R language. Also, we will show how you can use various functions provided by the sdsmR package to accelerate this process. 

# Exploratory Data Analysis 

## Accessing the data
Before we do anything, we must load the Blogsville data into R's workspace. Loading data into R is heavily dependent on your data-set, and there isn't an exact recipe for accessing every dataset you will encounter into R. From this, we will include instructions on how to load the Blogsville dataset into R manually in a supplement of this document, to get a flavor for how this is done. For convinience, we use the blogsville dataframe which comes pre-installed with the sdsmR package to walk through the steps of statistical downscaling. 

A Dataframe is a very useful data structure in the R Language. The fact that the pre-installed blogsville data-set is a dataframe  (verify this by typing `class(blogsville)`) object in R is very important. Most of the commands and functionality of the sdsmR package are designed to work specifically for dataframes. If you are working through this tutorial with a different dataset, the best first step would be to combine all of your data into a single dataframe object. This will allow you to work through the tutorial smoothly and utilize all the functionality of sdsmR. 
...
Note that if you're using your own data-frame, you can use the `quality_control()` function, which checks to make sure you're data-frame has all the required components for use in the sdsmR functions. 
...
1. Scatterplots of the relationship between an individual predictor and predictand. 

Creating these first two plots is a bit tricky. Due to this, we have created a function, called generate_table, which can be used to create these informative tables. The structure of this function is that you input a filepath, dataframe, and the name of the predictand variable, and as an output you will recieve two pdf files with the first two tables, in the filepath specified as an input. For more detailed information and examples on how to use this function, access the help file with `?generate_table`. 
...
Next, let's consider how to create scatterplots of individual predictor/predictand relationships. If you look at the explained variance table created by the generate_table function, you will notice that out of all the predictor variables, humidity (humxx) has the highest correlation with maximum temperature. The following code shows how to create a scatterplot of humidity against maximum temperature:
...
# Model Calibration
Next, we will see how to use sdsmR and the blogsville data-set to build a statistical model. This is the same step as the "Calibrate Model" section of the SDSM tool, and we will use the same predictor variables used in the manual. We also follow the convention from the blogsville example in the SDSM manual by splitting our data into training and testing sets, using the former for model building and the latter for validation. 

First, we show how to fit a annual model using the blogsville example, and that the values derived from this are identical to what you would have obtained from the SDSM tool. At the bottom of the following code chunk, we print the r squared and standard error from the linear model fit in R, and then print a screen shot of the same model in SDSM. 
...
![test](/home/lee/Dropbox/work/ncar/outputs/images/sdsm_calibrate.PNG)

One issue with the annual model for blogsville is that we see from figure above that temperature is heavily dependent on the time of year. In SDSM, one usually combats this by fitting seperate models either by season, or by month. In the next section, we will show how to do this both manually and with the `?calibrate_model` function. 

## Monthly and Seasonal Models 

Now that we have seen how to fit our annual model using R, let's delve into how we can create more complicated models. In SDSM, one of the most important options is the ability to specify whether you want annual, monthly, or seasonal models. If you choose a monthly or seasonal model, SDSM calibrates a seperate linear model for each time period. For example, if you select monthly, SDSM will return a seperate linear model for january, february, march, etc... Carrying out this step in R is a bit tricky, but certainly feasible. We will first show you how to manually fit both monthly seasonal models in R, then we will introduce the `calibrate_model` function, which will automate this step for you!
...
# Take the monthly and seasonal models out of the blogsville dataframe 
blogsville <- blogsville[, 1:8]
```
...
An alternative to using the code above to store lists of linear models is the `calibrate_model` function, which comes with the sdsmR package. It works the same way as above, storing each model in a list. You will notice that the output from running this function is the same we would have obtained from avoe. We can pull this informartion out with the `?summarize_models` function, which also comes with the sdsmR package. 
...
# Weather Generator 
Now that we can calibrate our models, we will show how to produce the equivalent of the weather generator section of the SDSM tool. Generally, this section is used to verify calibrated models, so we will use our testing data-set defined above to see how well our models perform. Recall that we split the blogsville data-set into a training and testing halves, the first from 1961-1975, and the second from 1976-1990. We build our model using the training data, and will now evaluate our model using the test set. 

SDSM adds white noise, ie: a normal random variable, onto all of its predictions in order to create ensembles. The reasoning behind generaing these ensembles is to closer match the variance of the observed and downscaled distributions. We will show how to generate these ensembles manually using our annual model, and then show how the `?generate_weather` function can execute this step for us. 
...
weather <- predict.lm(blogsville_annual_model, 
...
Similar to the  creation of the `calibrate_model` to mimic the Calibrate Model section, we have created the `generate_weather`, which mimics the functionality of the above code chunk for models calibrated by `calibrate_model`. With `?generate_weather`, you can use the output from the `calibrate_model` function and a new dataframe as inputs, and as an output recieve a dataframe with the dates a date column, exact predictions, and specified number of ensembles. For more detailed information on generate weather, access the `?generate_weather` help file. 
...
Next, we present a different way of quantifying the uncertainty in our downscaled predictions. Instead of adding white noise to the pointwise predictions, we use a prediction interval which comes straight from the regression equation. The reason to use the interval approach instead of the ensembles is that it gives coverage guarantees: approximately 95 percent of the prediction intervals will contain the true value. In the following section, we show the R code which generates both the predictions and corresponding prediction intervals. Recall that the actual predictions are the same whether you're generating predictions intervals or using ensembles, but what is different is how we are quantifying the uncertainty. 
...
plot(test$dates[1:365], weather[1:365], type = "l", 
     ylim = c(0, 35), lwd = 3, 
     main = "Ensembles 1975 in Blogsville", 
...
Finally, we arrive at the scenatio generation section where we use GCM output to downscale local variables. In this case, we are using predictors from the HadCM3 experiment, the same variables used in the Blogsville SDSM manual. Note that the process of generating scenario's from climate model output is mechanically the same as the `generate_weather` function we used in the previous section. However, instead of using a testing set of the NCEP re-analysis data to verify model output, we are using climate model output to guide the forecast. Since the calculations are the same, we can use the `generate_weather` function to calculate the different scenario's. A key requirement is that you must have a dataframe with the same predictor variable names that used for model calibration. Below we will generate the scenario's from both 1961-1990 and 2070-2099. We will show that, as expected from climate change, maximum temperatures are expected to be higher on average in Blogsville in the 2070-2099 time period. In the same way as the blogsville data, the climate model output comes with the sdsmR package, so we can just load it into R's workspace automatically. 
...
For stage two, we subset the data to only include days in which there is precipitation (ie: $W_{i}$ = 1). Next, call the amount of precipitation on day i $P_{i}$. Using the same predictor variables $X_{i}$ we used to fit the linear regression model in stage one, fit a linear model to $P_{i}$. And that's it, here is the code which performs those first two steps:
