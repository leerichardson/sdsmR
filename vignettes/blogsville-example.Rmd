...
In this document, we will walk through how to perform statistical downscaling, such as the kind performed by the Statistical Downscaling Model (SDSM) tool, in the R language. Statistical downscaling is a method of relating large scale variables, such as those from General Circulation Model (GCM) output, to local, typically pointwise variables. Statistical downscaling provides a method for performing local impact studies, such as assessing the risk of flooding in a particular city under different climate change scenario's. At a high level, what's being done is building a statistical model of the relationship between large scale variables (for example, temperature in a 50 x 50 km gridbox) and local variables using re-analysis data, and then using this statistical relationship to generate scenario's for how the local variable responds to various climates. 

We will walk through the Blogsville example used in the SDSM manual, and show how you can perform these calculations in the R language. Also, we will show how you can use various functions provided by the sdsmR package to accelerate the process. 
...
That the pre-stalled blogsville data-set is a dataframe object in R is very important. Most of the commands and functionality of the sdsmR package are designed to work specifically for dataframes. If you are working through this tutorial with a different dataset, the best first step would be to combine all of your data into one dataframe object. This will allow you to work through the tutorial smoothly and use all of the functionality of sdsmR. 
...
# Model Calibration
Next, we will see how to use sdsmR and the blogsville data-set to build a statistical model. This is the same stage as the Calibrate model section of the SDSM tool, and we will use the same predictor variables used in the manual. We also follow the convention from the blogsville example in the SDSM manual by splitting our dat into training and testing datasets, using the former for model building and the latter for validation. 

First, we will show how to fit a annual model using the blogsville example, and that the values derived from this are identical to what you would have obtained from the SDSM tool. 
...
# Weather Generator 
Now that we can calibrate our models, we will show how to produce the equivalent of the weather generator section of the SDSM tool. Generally, this section is used to verify calibrated models, so we will use our testing data-set defined above to see how well the model performs. Recall that we split the blogsville data-set into a training and testing halves, the first from 1961-1975, and the second from 1976-1990. We build our model using the training data, and will now evaluate our model using the test set. 

SDSM adds white noise, ie: a normal random variable, onto all of its predictions in order to create ensembles of weather predictions. The reasoning behind generaing these ensembles is to closer match the variance of the observed and downscaled distributions. Next, we will show how to generate these ensembles manually using our annual model, and then show how to use the `?generate_weather` function to perform the same calculations. 
...
Similar to the Model Calibration section, we have created an R function, `generate_weather`, which mimics the functionality of the above code chunk for models generated with `calibrate_model`. With this function, you can use the output from the calibrate_model function and a new dataframe as inputs, and you will recieve a dataframe with the dates, exact predictions, and specified number of ensembles. For more detailed information on generate weather, access the `?generate_weather` help file. 
...
As we can see, both options do a fairly good job providing a range of weather to expect in future years. However, we do notice that niether does a great job capturing the extremely hot days, which occur around the middle of July. This is an issue with statistical downscaling, that it doesn't do a great job capturing extreme values. We will see more of this theme in the conditional models section. 

# Scenario Generation 

Finally, we arrive at the main section where we use GCM output to downscale local variables. In this case, we are using predictors from the HadCM3 experiment, the same variables included in the Blogsville SDSM manual. Note that the process of generating scenario's from climate model output is statistically the same as the `generate_weather` function we used in the previous section. However, instead of using a testing set of the NCEP re-analysis data, we are using climate model output to guide the predictions. Since the statistical calculation is the same, we can use the `generate_weather` function to calculate these different scenario's. The input requirements are that you must have a dataframe with the same predictor variable names that used in the model calibration stage. Below we will generate the scenario's from both 1961-1990 and 2070-2099, along with thir corresponding uncertainty. We will show that, as expected from climate change, maximum temperatures are expected to be higher on average in Blogsville in the 2070-2099 time period. Note that similar to the blogsville data, the climate model output comes with the sdsmR package, so we can just load it into R's workspace automatically. 
...
To demonstrate the utility of the package in performing statistical downscaling, let's look at how concise the statistical downscaling process can be made utilizing sdsmR. In this example, we will simply downscale maximum temperature, using a monthly model and an autoregressive term. 
...
# Calibrate Model -----------------
demo_mod <- calibrate_model(dataframe = train, y = "tmax", model_type = "monthly", autoregression = "true")

# Weather Generator ---------------
demo_test <- generate_weather(models = demo_mod, new_dataframe = test)
